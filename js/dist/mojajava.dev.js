...
napravimeni();
document.getElementById("bs-example-navbar-collapse-1").innerHTML = meni; //#features
//featurenaslov
...
var featurenaslovi = ["corak", featurenaslov1, featurenaslov2, featurenaslov3]; ///featuretekst
...
var featuretekst = ["corak", featuretekst1, featuretekst2, featuretekst3];
...
napravifeatures();
feature += '</div></div>';
document.getElementById("features").innerHTML = feature; //#restaurant-menu
//vreme

var trenutnovreme = new Date().getHours(); //jela i pica
...
var randombroj = Math.floor(Math.random() * (54 - 29 + 1)) + 29; //kreiranje meni-ja
...
document.getElementById("pica").innerHTML = menitekstpica; //forma

var proslave = ["Select", "Wedding", "Birthday", "Company party", "Masquerade"];
var pros;

for (var i = 0; i < proslave.length; i++) {
...
var uslovi = document.querySelector("#terms");
...
var novagalerija = document.createElement('div');
...
pravljenjegalerije2();
document.getElementById("galerija").appendChild(novagalerija); //kontakt
...
brojljudiselect();
brojljudi += '</div>';
document.getElementById("butoni").innerHTML = brojljudi; //regex

function provera() {
  var greskabrojac = 0;
  var ime, prezime, email, brojljudikon, proslava;
  ...
  var indexproslave = proslava.selectedIndex;
  var regimeprezime = /^[A-Z][a-z]{2,20}$/;
  var regemail = /^[a-z\d]+.(gmail.com)$/;
  var regtelefon = /^[0-9]{2,10}$/;
  ...
  if (!regimeprezime.test(prezime.value)) {
    ...
    prezime.nextElementSibling.innerHTML = "The last ime submitted is written incorrectly.";
    greskabrojac++;
  } else {
  ...
  for (var _i2 = 0; _i2 < brojljudikon.length; _i2++) {
    if (brojljudikon[_i2].checked != true) {
      console.log("1");
